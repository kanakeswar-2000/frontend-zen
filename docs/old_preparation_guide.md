...
> Your 12-week roadmap from hesitant to hired. Follow it like a training program — show up every day, do the reps, trust the process.

---

## 0. Your Existing Work — You've Already Built More Than You Think

Before we start, let's acknowledge something important: **you are not starting from zero.** You have two repositories full of completed work that prove you can build things. The plan below integrates these projects as review exercises, rebuild challenges, and confidence anchors.

### What You've Already Built (JS-and-React-Utility-apps)

**JavaScript Foundations (Feb 2020)**
- 90+ JS drill exercises across 3 assignment sets (variables, functions, operators, strings, arrays)
- A **calculator** with expression parsing (`js-assignments/a4-calculator`)
- A **signup form** with regex validation, password matching, gradient styling (`js-assignments/sigh-up-form`)
- An **RGB color panel app** with reorder/resize using flexbox manipulation (`js-assignments/rgb-app`)
- A **todo list** with add/complete/delete/filter + localStorage persistence (`js-assignments/todo-list`)
- A **water count tracker** with weekday selection + localStorage (`js-assignments/water-count`) — has a merge conflict to fix

**CSS & Layout (Feb–Mar 2020)**
- 6 **flexbox** assignments — from basic rows to holy-grail layouts (`assignments/flexbox-1` through `flexbox-6`)
- **CSS Grid** practice (`Css Grid/`)
- **CSS Animations** and transitions (`CSS Animations/`)
- 9 **Bootstrap** assignments — gradients, shapes, responsive patterns (`Css-Bootstrap/`)
- Responsive page clones: Gmail landing page, GitHub trending page, phone dialer UI, weather dashboard mockup (`assignment-11` through `assignment-20`)

**React (Mar–Apr 2020)**
- React UMD exercises — components rendered into multiple roots (`react/assignment-1`)
- A **cars fleet / clock** app — lifecycle, intervals, lists/keys, lifting state (`react/assignment-2`)
- A **temperature converter** — the classic React "lifting state up" example (`react/Practice/temperatures-lifting-up-state`)
- A **multi-route app** (`react/myapp`) with React Router v5: Home, Todo, Form components, and a **COVID-19 Dashboard** with country cards, filtering, detail view, and light/dark theme

### What You've Already Built (reactjs-projects)

This is a larger CRA project (~229 source files) containing:
- **Countries dashboard** with region filtering, search, themed UI (using a `withCountries` HOC)
- **Emoji memory game** — shuffle, track clicks, win condition
- **Grid memory game** with MobX theming
- **Multiple todo implementations** — local state, JSONPlaceholder API, MobX, MobX v2
- **E-commerce store** with product listing, cart, pagination, sign-in, and protected routes
- **Authentication flow** with MobX AuthStore, token management, StorageUtils
- **Form exercises** — greetings, dessert picker, city selector, state selector
- **HOC patterns** — `withCountries`, `withFormatCurrency`, toggle, collapse, device type
- **Render props** — mouse coordinates demo
- **API layer** with apisauce, service classes, fixtures
- **Storybook** stories for multiple components
- **Mixed TypeScript** integration into CRA
- **MobX** state management throughout

### How This Plan Uses Your Existing Work

| Plan Phase | How Your Old Projects Fit In |
|------------|------------------------------|
| **Week 1** (React Foundations) | Review your `react/myapp` COVID dashboard. Rebuild the card grid + detail pattern from memory — you did this before, now do it *fast* |
| **Week 2** (React Router) | Compare your Router v5 code in `myapp` to v7 syntax. The structure is the same — only the API changed |
| **Week 3** (TypeScript) | Convert your existing todo list or temperature converter to TypeScript as practice |
| **Week 5** (State Management) | Study your MobX stores in `reactjs-projects` — you already understand store concepts. Zustand is simpler. |
| **Week 6** (Advanced Patterns) | You already built HOCs and render props. Review them — hooks replace most of these, but understanding the progression matters for interviews |
| **Week 7** (UI Libraries) | Rebuild your weather dashboard mockup (assignment-20) using Shadcn — compare the effort |
| **Debugging** (ongoing) | Fix the merge conflict in `water-count.js` as a real-world debugging exercise on Day 5 |

> **The bottom line:** You've written 300+ files of frontend code across these two repos. You're not a beginner learning React — you're a developer who needs to get *faster* and more *confident* at what you already know how to do. This plan treats you accordingly.

### Quick Reference: Where Each Old Assignment Appears in the Plan

| Your Old Assignment | Where It Shows Up | How It's Used |
|---------------------|-------------------|---------------|
| `js-assignments/todo-list` | Day 2 | Compare vanilla JS DOM todo vs React todo — see the difference |
| `js-assignments/water-count` | Day 5 | Fix the real merge conflict in `water-count.js` as a debugging exercise |
| `js-assignments/a4-calculator` | Day 5 | Rebuild as a React app with proper state management |
| `js-assignments/sigh-up-form` | Day 45 | Compare raw regex/DOM validation to React Hook Form + Zod |
| `js-assignments/rgb-app` | Day 36 | Rebuild with React + useMemo for color computation |
| `assignments/flexbox-1 to 6` | Day 46 | Compare raw CSS/Bootstrap responsive to Tailwind utility classes |
| `assignment-11 to 20` (responsive pages) | Day 46 | Reference for responsive patterns — now done with Tailwind |
| `assignment-20` (weather dashboard mockup) | Day 22, Day 46 | Rebuild as a functional React app with API data + Shadcn |
| `react/assignment-1` (UMD components) | Day 1 | Review old class components, compare to modern functional components |
| `react/assignment-2` (cars fleet/clock) | Day 37 | Review lifecycle/intervals — now done with useEffect + useRef |
| `react/Practice/temperatures-lifting-up-state` | Day 23 | Classic lifting state example — now also consider Context |
| `react/myapp` (COVID dashboard) | Day 1, 4, 8, 10 | Your original list→detail pattern. Rebuild it faster with v7 Router |
| `reactjs-projects` — countries dashboard | Day 3, 10 | Compare HOC data fetching to useEffect/hooks approach |
| `reactjs-projects` — emoji game | Day 40 | Rebuild as a speed drill with modern hooks + TypeScript |
| `reactjs-projects` — e-commerce (Botique) | Day 29, 30 | Compare MobX stores to Zustand — same concepts, simpler API |
| `reactjs-projects` — todo (4 versions) | Day 29 | Study the progression: local → API → MobX → MobX v2 |
| `reactjs-projects` — HOCs & render props | Day 25 | Review old patterns, understand how custom hooks replace them |
| `reactjs-projects` — auth flow | Day 50–52 | Reference for protected routes in your GitHub Explorer project |
| `reactjs-projects` — Storybook | Day 53 | Reference for component documentation approach |
| `Css-Bootstrap/`, `CSS Animations/`, `Css Grid/` | Day 46, ongoing | Foundation skills — you already have these. Tailwind builds on them. |
...
- **Review your old work (30 min):** Open `react/myapp/src/App.js` and `react/assignment-1/index.js` from your JS-and-React-Utility-apps repo. Read through the code. Notice: you wrote React components before. You used class components and `ReactDOM.render`. You built a COVID dashboard with country cards. That's real work. Today you're upgrading to modern React (functional components, Vite, hooks) — but the *concepts* are the same.
...
- **Bonus:** Look at your old flexbox assignments (`assignments/flexbox-2` — the Twitter card). Rebuild that same card layout, but this time using React components + Tailwind instead of raw HTML/CSS. Notice how much cleaner it is. Target: 20 min.
...
- Write down: "What felt easy today? What slowed me down? How does my new code compare to what I wrote in 2020?"
...
- **Review your old work (20 min):** Open your vanilla JS todo list (`js-assignments/todo-list/todo-list.js`). Study how you handled add/delete/filter/localStorage with raw DOM manipulation. Notice how much boilerplate was needed — `document.createElement`, `appendChild`, template cloning, manual DOM updates. React replaces all of that.
...
**Afternoon Block (2.5 hours) — Building**
- Build a Todo List app in React from scratch (add, delete, mark complete, filter by status) — target: 60 min
  - **Compare:** Your old `js-assignments/todo-list` had the same features but with raw DOM. Your React version should be cleaner and faster to build. If it's not faster yet, that's fine — it will be by the end of this week.
...
- Timer on. Build a simple counter app with styled +/- buttons and a reset button. Target: 8 min. Then build a color-theme toggler (light/dark) using state. Target: 10 min.
  - **Context:** Your `reactjs-projects` repo has a counter page (`/counter-page`) and theme toggling (`ThemeContext`). You've done both before — now make them *fast*.

**End-of-Day Reflection**
- "How many components did I create today? How does this React todo compare to my vanilla JS one? What took 50 lines of DOM code before now takes ___ lines in React."
...
- **Review your old work (15 min):** Open `reactjs-projects/src/components/common/hocs/withCountries.js`. You fetched data from the REST Countries API using a HOC + `componentDidMount`. Today you'll learn the modern way: `useEffect` + `fetch`. The concept is identical — only the syntax changed.
...
- Build a User Directory: Fetch users from `https://jsonplaceholder.typicode.com/users`, display as cards with name, email, company. Add a search filter by name. Target: 45 min.
  - **Compare:** Your `reactjs-projects` has a `/users` route that does something similar with MobX stores and service classes. Your new version should be simpler — just `useEffect` + `useState` + `fetch`. No stores, no services, no class components.
- Add loading and error states (spinner while loading, error message on failure). Target: 15 min.
- Key practice: Before writing any code, spend 5 minutes looking at the API response in the browser/Postman. Map out which fields you need for the UI. This is the **API-to-UI planning** step you must internalize.
...
**End-of-Day Reflection**
- "How long did I spend looking at the API response before coding? Did I plan my component structure before writing JSX? How does `useEffect` compare to `componentDidMount` from my old code?"
...
- **Review your old work (20 min):** Open your COVID-19 Dashboard in `react/myapp/src/components/`. You built *exactly* this pattern before — a grid of country cards, click to navigate to `/covid19-dashboard/details/:countryId`. The PokeAPI challenge is the same architecture: grid of cards + detail route. You already know how to do this. Now do it with modern React and React Router v7.
...
- **Real debugging exercise — fix your old code (30 min):** Open `js-assignments/water-count/water-count.js`. This file has **unresolved git merge conflict markers** (`<<<<<<<`, `=======`, `>>>>>>>`). This is a real-world debugging scenario:
  1. Read the conflicting sections carefully
  2. Understand what each version of the code was trying to do
  3. Resolve the conflict by choosing the correct version (or merging both)
  4. Test the app in a browser to verify it works
  - This is more valuable than any artificial debugging exercise because it's *your own code*.
...
- **Bonus exercise:** Open your old `js-assignments/a4-calculator/calculator.js`. It uses `eval()` for expression parsing. Intentionally enter broken expressions and observe the try/catch handling. Then build a React version of this calculator with proper state management. Target: 30 min.
...
- **Side-by-side comparison (20 min):** Open `react/myapp/src/App.js` and `reactjs-projects/src/App.js`. Both use React Router v5 with `<Switch>`, `<Route component={...}>`, and `<HashRouter>`. Now read the v7 docs — the changes are: `<Switch>` → `<Routes>`, `component={X}` → `element={<X />}`, `useHistory()` → `useNavigate()`, `match.params` → `useParams()`. Write a cheat sheet mapping your old v5 syntax to v7 equivalents. This is specifically where you got stuck in the mock interview — nail it today.
...
- **You've done this before:** Your COVID dashboard had `/covid19-dashboard/details/:countryId`. Your countries dashboard in `reactjs-projects` had the same pattern. Today you rebuild it with v7 syntax and a new API.
...
- **Note:** Your `reactjs-projects` already has a `tsconfig.json` and some TypeScript files mixed in. You've been *around* TypeScript before, even if you didn't write it yourself. Open `reactjs-projects/src/index.tsx` — that's TypeScript. It's not as foreign as you think.
...
- **Review your old work (20 min):** Open `reactjs-projects/src/components/common/hocs/` — you already built HOCs like `withCountries` and `withFormatCurrency`. Also open the render props example (`MouseCoordinates`/`DisplayMouseCoordinates` under the practice-advanced-concepts route). You *know* these patterns. Today's learning is understanding that **custom hooks have mostly replaced HOCs and render props** in modern React. For each HOC you wrote before, think about how a custom hook would achieve the same thing more simply.
- Key patterns to understand:
  - Children as props (`children: React.ReactNode`)
  - Compound components (like `<Tabs>` + `<Tabs.Tab>` + `<Tabs.Panel>`)
  - Render props (less common now, but understand the concept — you built one before!)
  - Higher-order components (understand, don't use much — hooks replaced most HOCs. You built `withCountries`, `withFormatCurrency` before — now convert that thinking to hooks)
...
- Go back and look at your code from 2020 in `JS-and-React-Utility-apps`. Then look at what you wrote this week. Notice: the class components → functional components shift, the raw DOM → React shift, the untyped → TypeScript shift. You've grown more in 4 weeks than most developers do in 4 months.
...
**Morning Block (2.5 hours) — Theory**
- **Review your old work first (20 min):** Open `reactjs-projects/src/App.js` and look at the `stores` and `globalStores` objects passed to the MobX `Provider`. You already understand the *concept* of a store: a central place that holds state, exposes actions to modify it, and lets multiple components read from it. MobX uses `@observable` and `@action` decorators. Zustand (which you'll learn this week) uses the same mental model but with a much simpler API — no decorators, no providers, no class syntax. Also review your multiple todo implementations: `todo-list`, `todo-list-api`, `todo-list-mobx`, `todo-list-mobx-v2` — you already went through the progression from local state → API-backed → store-managed.
...
- **Context from your old work:** Your `reactjs-projects` has an e-commerce "Botique" store with products, cart, pagination, sign-in, and protected routes. That version used MobX `ProductStore` + `CartStore` + service classes. Today you start fresh with just `useState`. By Day 30, you'll add Zustand — and you'll see how much simpler it is than the MobX version you used before. Open `reactjs-projects/src/Botique/` briefly to remember the old approach.
...
**End-of-Day Reflection**
- "Where did prop drilling feel painful? Which pieces of state are used by distant components? How does this compare to the MobX approach in my old `reactjs-projects`?"
...
- Target: 90 min
- **Bonus rebuild:** Your old `js-assignments/rgb-app` manipulated flex order and background colors with raw DOM. Rebuild it as a React app where the color state and sort computations use `useMemo`. Target: 30 min.
...
- **Review your old work (15 min):** Open `react/assignment-2/cars-fleet.html`. Your cars fleet app used `componentDidMount` with `setInterval` to run live clocks. The interval ID was stored on the class instance (`this.timerId`). In modern React, you'd store that interval ID in a `useRef` — because it's a mutable value that shouldn't trigger re-renders. That's the core concept of `useRef` for non-DOM uses.
...
- Practice: Build a component that auto-focuses an input on mount. Build a "previous value" hook using useRef. Then rebuild the cars fleet clock concept using `useRef` for the interval ID + `useEffect` for cleanup.
...
**Morning Block (1 hour) — Planning**
- **Option A:** Plan a Lyrics Finder using a free lyrics API or a Music Discovery app using [Deezer API](https://developers.deezer.com/api) (via a CORS proxy like `https://corsproxy.io/`)
- **Option B (recommended):** Rebuild your **Emoji Memory Game** from `reactjs-projects`. Your old version used MobX + class components. Rebuild it from scratch with: functional components, hooks, TypeScript, Zustand for game state, Shadcn for UI. Add features your old version didn't have: difficulty levels, timer, best score tracking with localStorage, animations.
- Spec: Search/browse or game logic, favorites/scores, dark mode, fully typed, Zustand, custom hooks, code splitting

**Afternoon Block (5.5 hours) — Full Build**
- Build it from scratch. Use every pattern you've learned in Weeks 1–6.
- Target: Complete working version in 3 hours.
- **If you chose the Emoji Game:** Compare your new code side-by-side with the old `reactjs-projects` version. The difference in code clarity and simplicity is your growth made visible.
- Push to GitHub with README.
...
- **Review your old work (15 min):** Open your `js-assignments/sigh-up-form/sign-up.js`. You built form validation with raw regex, `onblur`/`onkeyup` events, and manual DOM error messages. Also look at your `reactjs-projects` form exercises (`/form-components/` route — greetings, dessert, cities, state selector). You know forms. React Hook Form + Zod replaces all that manual validation with a declarative schema — much cleaner.
...
- **Review your old work (20 min):** Open your responsive assignments from `JS-and-React-Utility-apps`: `assignment-14` through `assignment-18` (CSS media queries), `assignment-19` (phone dialer UI), and `assignment-20` (weather dashboard mockup). You built all of these with raw CSS media queries and Bootstrap. Now you'll achieve the same responsive behavior with Tailwind utility classes (`sm:`, `md:`, `lg:`, `xl:` prefixes) — much faster and no separate CSS files. Also look at your `assignments/flexbox-5` and `flexbox-6` — responsive flex layouts that you'll now do in seconds with Tailwind.
...
- **Reference your old work:** Before planning, review two things from `reactjs-projects`:
  1. The **auth flow** (`src/Authentication/`) — how `ProtectedRoute` checks `isLoggedIn()` via token. You can implement a similar pattern if you add a "save API key" feature for GitHub API rate limits.
  2. The **Botique e-commerce** routes (`src/Botique/routes/`) — how routes were organized in a separate `routes/index.js` file. Good pattern for larger apps.
  3. The **Storybook** setup (`.storybook/`) — consider adding Storybook to this project as a stretch goal for component documentation.
...
### Your Existing Projects (Reference, Not Portfolio)

Before listing the new projects, remember: you already have a foundation to build on.

| Old Project | What You Built | What's Missing for Modern Standards |
|-------------|----------------|-------------------------------------|
| COVID-19 Dashboard (`react/myapp`) | Card grid, filtering, detail route, dark mode | TypeScript, modern hooks, Router v7, Zustand, testing |
| Countries Dashboard (`reactjs-projects`) | HOC data fetching, region filter, search, theming | Same as above — class components, MobX, no tests |
| E-commerce Botique (`reactjs-projects`) | Products, cart, auth, protected routes | Heavy MobX, no TypeScript, no modern UI library |
| Emoji Game (`reactjs-projects`) | Game logic, shuffle, score tracking | Class components, MobX, no TypeScript |
| Todo List (4 versions in `reactjs-projects`) | Local → API → MobX → MobX v2 | Great progression but needs hooks + TypeScript update |

These projects prove you can build real things. The new projects below demonstrate that you can build them *better, faster, and with modern tools*.
...
- **Builds on:** Your COVID dashboard card grid + detail pattern from `react/myapp`
...
- **Builds on:** Your Botique e-commerce in `reactjs-projects` — same domain, modern approach (Zustand instead of MobX, hooks instead of classes)
...
- **Your old repos are proof you can code.** When doubt hits, open your `JS-and-React-Utility-apps` commit history. Between Jan 31 and Apr 15, 2020, you made 50 commits building real projects — a calculator, a todo app, a water tracker, a COVID dashboard, form validation, responsive layouts. You built a 229-file React app with routing, MobX, e-commerce, and authentication. You are *not* a beginner. You are a developer who needs to get faster.
